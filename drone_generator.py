...
class Builder:
    def __init__(self,
        ...
        # The algorithm for preparing the data-set.
    ...
    def run(self, testing_mode=False, testing_mode_generation_limit=10):
        ...
        for config in self.configurations:
            ...
            if not accept_config:
                ...
                _, _, _, f0 = self.frame_grabber(v)

                # resize d with respect to s0
                d0, drone_w, drone_h = self.load_and_resize(d, s0)

                # overlay f0 with d in position p0
                d0_loc, f0, d0_shape = self.paste_object(d0, f0, p0)

                # draw (p1, s1, f1) in the same way
                p1, s1 = self.get_pos_and_size(g, s)
                _, _, _, f1 = self.frame_grabber(v)

                # resize d with respect to s1
                d1, d1width, d1height = self.load_and_resize(d, s1)

                # overlay f1 with d in position p1
                d1_loc, f1, d1_shape = self.paste_object(d1, f1, p1)

                # draw (p2, s2, f2) in the same way
                p2, s2 = self.get_pos_and_size(g, s)
                _, _, _, f2 = self.frame_grabber(v)

                # resize d with respect to s2
                d2, d2width, d2height = self.load_and_resize(d, s2)

                # overlay f2 with d in position p2
                d2_loc, f2, d2_shape = self.paste_object(d2, f2, p2)
                ...
                bird0, _, _ = self.load_and_resize(b0, sb0)
                ...
                bird1, _, _ = self.load_and_resize(b1, sb1)

                # overlay f1 with b0 in position pb,0
                _, f1, _ = self.paste_object(bird0, f1, pb0)

                extra_bird = np.random.choice([True, False])
                if extra_bird:
                    # overlay f1 with b1 in position pb,1
                    _, f1, _ = self.paste_object(bird1, f1, pb1)

                # grab random frame from v f4
                _, _, _, f3 = self.frame_grabber(v)

                # overlay f4 with b0 in position pb,0
                _, f3, _ = self.paste_object(bird0, f3, pb0)

                extra_bird = np.random.choice([True, False])
                if extra_bird:
                    # overlay f3 with b1 in position pb,1
                    _, f3, _ = self.paste_object(bird1, f3, pb1)
