...
def paste_object(object_to_paste, background, position):
    ...
    try:
        for c in range(0, 3):
            ...
            background[y1:y2, x1:x2, c] = (alpha_object[:bg_region[0], :bg_region[1]] * object_to_paste[:bg_region[0], :bg_region[1], c] +
        ...
        err_str = "pos: (y,x) (%s) \n"\
                  ...
                  "background shape: %s" % (position,x_offset,y_offset, x1, x2, y1, y2, alpha_object.shape, alpha_background.shape,object_to_paste.shape, background.shape)
        print(err_str)
...
def load_and_resize(obj_file_name, target_size):
    # 13 resize object with respect to size, size becomes smaller side of object
